...
public class TestsSteps {
    ...
    static public void stepCloseNotifyPage() {
       // step("Закрываем окно с предложением подписаться на оповещения", () -> {
            try {
                ...
                System.out.println(err+"Закрываем окно с предложением подписаться на оповещения");
            }
       // });
    }
    ...
    static public boolean selectProductSize(String loc) {
        if (notVisible(loc,10)) stepCloseNotifyPage();
        ...
        if (sizeCount > 0) {
            ...
            if (notVisible(loc,20)) stepCloseNotifyPage();
    ...
    static public boolean notVisible(String loc,int seconds){
        while (!$(loc).is(visible)){
            sleepMs(1000);
    ...
    static public String stepAddOneProductToCart() {
        ...
        while (true) {
            ...
            step("Загружается страница товара", () -> {
                ...
                if (notVisible(".button.product__button-cart",10)) stepCloseNotifyPage();
            ...
            step("Выберем случайное количество товара \"" + prod + "\": " + numberRnd, () -> {
                ...
                while (i < numberRnd) {
                    ...
                    if ($$("button.field__plus").filterBy(visible).size() != 0) {
                        i++;
                        if ($$("button.field__plus").filterBy(visible).size() != 0) {
                            clicked = true;
                            if (i > 1) {
                                if (notVisible("button.field__plus",10)) stepCloseNotifyPage();
                                $$("button.field__plus").findBy(visible).scrollTo().click();
                            }
                        }
                    }
                    if (!clicked) {
